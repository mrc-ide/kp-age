...
survey_id <- read_csv("src/0_extract_raw/survey_paths.csv") %>% 
  filter(survey_id == "COG2017BBS_PWID") %>% 
  pull(survey_id) %>% unique()
...
saveRDS(filteredcleandata, "~/Imperial College London/HIV Inference Group - WP - Documents/Data/KP/Individual level data/00Admin/Data extracts/agehiv_data_extract_1802.rds")
