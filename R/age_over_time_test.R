...
fsw_age_formula1 <- age ~ 1 + mean_year + f(iso3, model = "iid") + f(iso3_id, mean_year, model = "iid") + offset(log(genpop_mean))
...
sd_fsw <- data.frame(year = 1993:2023,
                     denom = 1) %>% 
  bind_rows(fsw_age_dat %>% group_by(survey_id) %>%
  mutate(mean_kp_age2 = mean(age, na.rm = T), 
         sd_kp_age = sd(age, na.rm = T)) %>% 
  ungroup() %>% 
  select(survey_id, iso3, year, mean_kp_age, sd_kp_age, genpop_mean, megamean, denom) %>% 
  distinct() %>% 
  left_join(genpop_median_ages %>% filter(sex == "female") %>% select(iso3, year, sd_age)) %>% 
  mutate(mean_diff = mean_kp_age - genpop_mean,
         sd_diff = sd_kp_age - sd_age)) %>% 
  mutate(mean_year = year - 2014)


sd_formula = sd_diff ~ mean_year + f(survey_id, model = "iid")


sd_mod <- INLA::inla(formula = sd_formula,
                                      family = "gaussian",
                                      control.compute=list(config = TRUE),
                                      data = sd_fsw)
...
sd_samples <- moz.utils::sample_model(sd_mod, sd_fsw, col = "survey_id")

sd_samples %>% 
  ggplot() + 
  geom_line(aes(x = year, y = mean)) + 
  geom_ribbon(aes(x = year, ymin = lower, ymax= upper), alpha = 0.3, fill = "darkgreen") +
  # geom_point(aes(x = year, y = sd_kp_age, size = denom)) +
  # geom_smooth(aes(x = year, y = sd_kp_age, weight= denom), method = "lm")
  geom_point(data = sd_fsw, aes(x = year, y = sd_diff, size = denom)) +
  # geom_smooth(aes(x = year, y = sd_diff, weight = denom), method = "lm") +
  labs(y = "Difference in SD") +
  theme_minimal()



sd_msm <- data.frame(year = 2010:2023,
                     denom = 1) %>% 
  bind_rows(msm_age_dat %>% ungroup() %>% group_by(survey_id) %>%
              mutate(mean_kp_age2 = mean(age, na.rm = T), 
                     sd_kp_age = sd(age, na.rm = T)) %>% 
              ungroup() %>% 
              select(survey_id, iso3, year, mean_kp_age, sd_kp_age, genpop_mean, megamean, denom) %>% 
              distinct() %>% 
              left_join(genpop_median_ages %>% filter(sex == "male") %>% select(iso3, year, sd_age)) %>% 
              mutate(mean_diff = mean_kp_age - genpop_mean,
                     sd_diff = sd_kp_age - sd_age)) %>% 
  mutate(mean_year = year - 2017)
...
sd_mod_msm <- INLA::inla(formula = sd_formula,
                     family = "gau ssian",
                     control.compute=list(config = TRUE),
                     data = sd_msm)
...
sd_samples_msm <- moz.utils::sample_model(sd_mod_msm, sd_msm, col = "survey_id")

sd_samples_msm %>% 
  ggplot() + 
  geom_line(aes(x = year, y = mean)) + 
  geom_ribbon(aes(x = year, ymin = lower, ymax= upper), alpha = 0.3, fill = "purple") +
  # geom_point(aes(x = year, y = sd_kp_age, size = denom)) +
  # geom_smooth(aes(x = year, y = sd_kp_age, weight= denom), method = "lm")
  geom_point(data = sd_msm, aes(x = year, y = sd_diff, size = denom)) +
  # geom_smooth(aes(x = year, y = sd_diff, weight = denom), method = "lm") +
  labs(y = "Difference in SD") +
  theme_minimal()
