...
survs <-  read_csv("~/Imperial College London/HIV Inference Group - WP - Documents/Data/KP/Individual level data/00Admin/Data extracts/agemoddat_20_06.csv") %>% 
  ...
  single_year_to_five_year(fifteen_to_49 = T) %>% 
...
age_groups <- naomi.utils::get_age_groups()
pop <- readRDS("../fertility_orderly/global/pop.rds")$MWI
...
dat <- survs %>%
  ...
  mutate(id.survey = factor(as.numeric(factor(survey_id))))
...
Z_survey <- sparse.model.matrix(~0 + id.survey, dat)
...
tmb_int$data <- list(   
  ...
  R_survey = as(diag(1, nrow = ncol(Z_survey)), "sparseMatrix"),
...
tmb_unload("tmb_comparison_trick")
lapply(list.files("~/Downloads", pattern = "\\.o|\\.so", full.names = T), file.remove)

TMB::compile("~/Downloads/tmb_comparison_trick.cpp", flags = "-w")

dyn.load(dynlib("~/Downloads/tmb_comparison_trick"))



f <- parallel::mcparallel({TMB::MakeADFun(data = tmb_int$data,
                                          parameters = tmb_int$par,
                                          DLL = "tmb_comparison_trick",
                                          silent=0,
