...
cfsw_formulae <- list(mod1 = n ~ 1 + id.age + f(id.iso3, model = "besag", graph = national_adj()),
                      mod2 = n ~ 1 + id.age + f(id.survey_id, model = "iid"),
                      mod3 = n ~ 1 + f(id.age, model = "ar1") +  f(id.survey_id, model = "iid"))

cfsw_mods_labs = data.frame(
  mod_name = c("mod1", "mod2", "mod3"),
  formula = c("Linear Age + Country ICAR", "Linear Age + Survey IID", "Age AR1 + Survey IID") 
)

cfsw_mods <- lapply(cfsw_formulae, run_inla_model_msm, data = cfsw_inla_dat)

all_samples_cfsw_mods <- imap_dfr(cfsw_mods, ~ {
  .x$samples %>%
    mutate(mod_name = .y)
}) %>% 
  left_join(cfsw_mods_labs)

cfsw_mods$mod1$summary
cfsw_mods$mod2$summary
cfsw_mods$mod3$summary

cfsw_inla_dat %>% filter(!(is.na(survey_id))) %>%
  select(survey_id, iso3, year, kp_prev, age, kp_odds, denom) %>%
  left_join(
    all_samples_cfsw_mods%>% 
