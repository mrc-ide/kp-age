...
cfsw_age_formula1 <- age ~ 1 + mean_year + f(iso3, model = "iid") + f(survey_id, model = "iid") + offset(log(genpop_mean))
...
summary(cfsw_agemod_gamma_offset)
...
(cfsw_agecount_gamma_offset <- cfsw_agemod_gamma_offset_samples %>%
    ...
    geom_line(data = genpop_median_ages %>% filter(year >1997, sex == "male") %>% select(year, megamean) %>% distinct(), aes(x = year, y = megamean), color = "darkred", linetype = "dashed", linewidth = 0.75) +
...
(cfsw_ageratio_gamma_offset <- cfsw_agemod_gamma_offset_samples %>% 
    ...
    geom_point(data = cfsw_age_dat , aes(x = year, y = mean_kp_age-genpop_mean, size = denom, color = iso3), show.legend = F) + 
