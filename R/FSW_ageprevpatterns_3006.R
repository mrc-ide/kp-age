...
data_prep <- fsw_dat %>%
  ...
  # age %in% 15:49) %>%
  left_join(read_sf(moz.utils::national_areas()) %>% select(iso3, id.iso3) %>% st_drop_geometry()) %>%
  left_join(pred %>% filter(!is.na(id.iso3.age)) %>% distinct(iso3, age_group, id.iso3.age)) %>% 
  mutate(id.age_group = factor(multi.utils::to_int(age_group)))
...
n_ages <- length(unique(pred$age_group))
...
#       rankdef = n_years + n_ages - 1L) ,
  ...
  mod4 = n ~ 1 + f(id.year, model = "rw2") + f(id.age_group2, model = "ar1") + f(id.iso3, model = "besag", graph = national_adj()) +
