...
//   ///////////// Time * Age - seeing what happens when we revert to older version
DATA_SPARSE_MATRIX(Z_periodage);
// DATA_SPARSE_MATRIX(R_period);

PARAMETER_ARRAY(eta2);
PARAMETER(log_prec_eta2); // this needs to go back to sigma, make sure to swap 151 for 152
// PARAMETER(logit_eta2_phi_age);
// PARAMETER(logit_eta2_phi_period)
PARAMETER(lag_logit_eta2_phi_age); //recomment these back in above the two above
PARAMETER(lag_logit_eta2_phi_period) //recomment these back in
