  ...
  
{
  ...
  // nll -= dnorm(lag_logit_eta2_phi_age, Type(0), Type(1), true);
  ...
  // nll -= dnorm(lag_logit_phi_beta, Type(0), Type(1), true); //added this
  nll -=dnorm(lag_logit_phi_beta, Type(0), Type(sqrt(1/0.15)), true); //pulled this out
  ...
  // 
  // // Replace sigma_eta3 with these two lines if you want log sigmaision prior 
  // Type sigma_eta3 = exp(log_sigma_eta3);
  // nll -= dgamma(sigma_eta3, Type(1), Type(20000), true);
  // 
  // // Type sigma_eta3 = exp(log_sigma_eta3);
  ...
  // 
  // // Commented out in favour of lag logit
...
// //   ///////////// Time * Age as it should be as of Dec 2024
...
// 
//   // nll -= dnorm(lag_logit_eta2_phi_age, Type(0), Type(1), true); // random additions
//   // nll -= dnorm(lag_logit_eta2_phi_period, Type(0), Type(1), true); // random additions
//   //   
//     //
//     Type sigma_eta2 = exp(log_sigma_eta2);
//   // nll -= dgamma(sigma_eta2, Type(1), Type(20000), true);
...
//   vector<Type> eta2_v(eta2);
  ...
  // PARAMETER(log_sigma_eta_surv);
  // PARAMETER(logit_eta_surv_phi_age);
  // // PARAMETER(lag_logit_eta_surv_phi_age);
  // //
  // Type sigma_eta_surv = exp(log_sigma_eta_surv);
  // nll -= dgamma(sigma_eta_surv, Type(1), Type(2000), true);
  // //
  // // // nll -= dnorm(logit_eta3_phi_age, Type(3.66116349), Type(0.09653723), true);
  // //
